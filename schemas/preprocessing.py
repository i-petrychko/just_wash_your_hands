...
class PreprocessingConfig(BaseModel):
    out_channels: int = Field(
        3, description="Number of output channels for preprocessing"
    )
...
class Config(BaseModel):
    ...
    preprocessing: PreprocessingConfig = Field(
        PreprocessingConfig(), description="Preprocessing settings"
    )
    paths: Optional[PathsConfig] = Field(None, description="Paths configuration")
    train_split: Optional[SplitConfig] = Field(None, description="Split configuration")
    filtering: Optional[FilteringConfig] = Field(
        None, description="Filtering configuration"
    )
    seed: int = Field(None, description="Seed for reproducibility")
