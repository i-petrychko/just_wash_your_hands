...
class ObjectSchema(BaseModel):
    ...
    def from_initial_name(
        cls, initial_name: str, config_path: str = "preprocessing/config.yaml"
    ):
        """
    ...
    @classmethod
    def from_name(
        cls, name: str, config_path: str = "preprocessing/config.yaml"
    ):
        """
        Create an ObjectSchema instance using initial_name and load the corresponding data from the config.
        """
...
class LabelSchema(BaseModel):
    ...
    def get_xywh(self) -> List[int]:
        if not self.coordinates:
            return []

        x = self.coordinates.min_x
        y = self.coordinates.min_y
        width = self.coordinates.max_x - self.coordinates.min_x
        height = self.coordinates.max_y - self.coordinates.min_y
